...
neg.words <- read.table("negativew.txt")
pos.words <- read.table("positivew.txt")
# Database Stuff
devdb <- dbConnect(RSQLServer::SQLServer(), server="localhost", port=1433,
                   properties=list(user="rdata", password="password"))
if(!exists(comments) || nrow(comments) == 0)
   {
     comments <- dbGetQuery(devdb, "select * from Comments")
}
...
reddit.sentiment <- score.sentiment(sentences, pos.words, neg.words, .progress='none')
